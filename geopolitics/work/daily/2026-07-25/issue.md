<!-- daily-issue-v1 inputs-sha256: b45dc66d5a7c49fe5e1c4cd9abcff71030f3da87700472122f906ba33962f35c -->
<!-- reality-lattice-sha256: e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855 -->
<!-- Generated file. Edit canonical daily or reality-lattice inputs, then regenerate. -->

Publication: **Narrative Geopolitics — Daily Issue**

# Chokepoints Are Becoming the War’s Main Weapon

Title standard: `reader-facing`

Title rationale: `The day’s sources converge on maritime and energy access as the mechanism that is constraining escalation.`

Date: `2026-07-25`

Status: `internal-reader-facing`

Evidence posture: `archive-first; story-level labels govern factual use`

## Front Page

The day’s source set points to a common constraint rather than a settled battlefield fact: maritime and energy access is becoming the principal leverage surface. If the reported closures and strikes persist, the next decision will be whether force can reopen the routes without consuming the defensive stocks and political room needed for a wider war. Because the underlying event claims remain unverified here, the immediate analytical priority is route status, force movement, and independent confirmation of infrastructure damage.

| Desk | Story | Evidence posture |
| --- | --- | --- |
| Main Analysis | Chokepoints are becoming the war's main weapon | `bounded-analysis` |

## Briefing Desk

No secondary brief met the issue threshold for this archive day.

## Main Analysis

### Chokepoints are becoming the war's main weapon

Evidence posture: `bounded-analysis`

Crisis object: Can force reopen maritime-energy access without widening the war?

Related reality claims: None registered

The day’s source set points to a common constraint rather than a settled battlefield fact: maritime and energy access is becoming the principal leverage surface. If the reported closures and strikes persist, the next decision will be whether force can reopen the routes without consuming the defensive stocks and political room needed for a wider war. Because the underlying event claims remain unverified here, the immediate analytical priority is route status, force movement, and independent confirmation of infrastructure damage.

## Source Ledger

Only sources used by selected issue stories appear here. See the [complete canonical source accounting](sources.md).

| Source ID | Date | Title | URL | Voice | Archive source | Analytical job |
| --- | --- | --- | --- | --- | --- | --- |
| `SRC-01` |  |  |  | Mercouris | [2026-07-25 Mercouris](../../../archive/sources/2026-07-25/source-black-sea-now-russian-lake-west-s-grain-gamble-backfired-2026-07-25.md) | Black Sea Now Russian Lake, West's Grain Gamble Backfired |
| `SRC-02` |  |  |  | Kent | [2026-07-25 Kent](../../../archive/sources/2026-07-25/source-joe-kent-iran-trap-u-s-ground-invasion-israeli-deception-2026-07-25.md) | Joe Kent: Iran Trap, U.S. Ground Invasion & Israeli Deception |
| `SRC-03` |  |  |  | Johnson | [2026-07-25 Johnson](../../../archive/sources/2026-07-25/source-larry-johnson-yemen-slams-saudi-oil-as-iran-and-the-us-prepare-for-massive-moves-2026-07-25.md) | Larry Johnson: Yemen SLAMS Saudi Oil as Iran and the US Prepare for MASSIVE MOVES |
| `SRC-04` |  |  |  | Mercouris | [2026-07-25 Mercouris](../../../archive/sources/2026-07-25/source-russia-strike-smashes-kiev-drone-fair-zelensky-angry-us-orders-kiev-stop-black-sea-attacks-kharkov-2026-07-25.md) | Russia Strike Smashes Kiev Drone Fair Zelensky Angry; US Orders Kiev Stop Black Sea Attacks; Kharkov |
| `SRC-05` |  |  |  | Marandi | [2026-07-25 Marandi](../../../archive/sources/2026-07-25/source-seyed-m-marandi-aramco-facility-engulfed-in-flames-after-yemeni-missile-strike-2026-07-25.md) | Seyed M. Marandi: Aramco Facility Engulfed in Flames After Yemeni Missile Strike |
| `SRC-06` |  |  |  | Weichert | [2026-07-25 Weichert](../../../archive/sources/2026-07-25/source-trump-preparing-massive-attack-on-iran-next-week-w-brandon-weichert-2026-07-25.md) | TRUMP PREPARING "MASSIVE ATTACK" ON IRAN NEXT WEEK - w/ Brandon Weichert |

## Forecast Desk

Forecasts remain accountable to the [canonical daily review](forecast.md) and [central ledger](../../forecasts/forecast-ledger.md).

No selected story carries a forecast hook.

## Verification Desk

Operational claims remain attributed unless canonical multilingual adjudication supports factual adoption.

### Missing observables

No additional missing observables were recorded.

## Voices / Columns

Names identify source-bounded pressure tests, not endorsement or reconstructed present opinion.

| Voice | Analytical operation | Contribution | Main risk |
| --- | --- | --- | --- |
| Mercouris | maritime/economic mechanism | Odessa closure and grain/energy exposure | attribution risk |
| Kent | force-planning constraint | ground options do not solve the end-state problem | unverified operational claims |
| Johnson | military logistics | command and stockpile pressure | insider-source dependence |
| Marandi | regional deterrence | Yemen/Iran agency and legitimacy | strong normative framing |
| Weichert | escalation pathway | pressure for a larger U.S. move | forecast-heavy |

## Editor's Note

This issue selects stories by declared analytical consequence, not by source volume.
- **Chokepoints are becoming the war's main weapon:** Unifies the day's maritime, energy, and force-constraint evidence

This is intentionally a thin issue: fewer than three story objects cleared the selection threshold.

Canonical inputs: [sources](sources.md), [synthesis](synthesis.md), [forecast](forecast.md), and [daily brief](daily-brief.md).

### Revision Log

No revisions.
