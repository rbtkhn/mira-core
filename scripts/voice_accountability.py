...
def render_markdown(ledger: dict[str, Any]) -> str:
    ...
    classes = ledger.get("allowed_classes", [])
    lines = [
        ...
        "",
        "Near-misses and representative exclusions: "
        "[voice-revision-near-misses.md](voice-revision-near-misses.md)",
        "",
        "## Inclusion Contract",
        "",
        ...
        *(f"- `{value}`" for value in classes),
        "",
        "Near-misses never enter this ledger. Confirmed entries may be mirrored into "
        "voice pages under `Accountability Notes`; the JSON ledger remains canonical.",
        "",
        "## Entries",
        "",
    ...
    for entry in entries:
        title = str(entry["source_title"]).replace("|", "\\|")
        source = str(entry["source_path"]).split("narrative-geopolitics/", 1)[-1]
        lines.append(
            f"| `{entry['id']}` | `{entry['date']}` | `{entry['voice_slug']}` | "
            f"`{entry['host_slug']}` | `{entry['class']}` | `{entry['status']}` | "
            f"[{title}](../../{source}#L{entry['line']}) |"
        )
    ...
    for entry in entries:
        ...
        lines.extend(
            [
                ...
                f"- Line reference: `{entry['line']}`",
                f"- Prior view: {entry['prior_view']}",
...
def validate_ledger(
    *,
    ...
    if markdown_text != rendered:
        failures.append("voice-revision Markdown drift from canonical JSON")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    render = subparsers.add_parser("render", help="Render Markdown from canonical JSON")
