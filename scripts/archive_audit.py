    ...
    *,
    ...
    for row in rows:
        ...
        if metadata.get("routing_state") == "provisional":
            findings.append(finding("routing.provisional", "warning", normalized, "source routing remains provisional"))
    ...
    scoped_dates = {value.isoformat() for value in _iter_dates(scope.effective_start, scope.effective_end)}
    for path in sorted(sources_root.rglob("*.md")):
        relative = path.resolve().relative_to(repo_root.resolve()).as_posix()
        match = DATE_IN_PATH.search(relative)
        if match and match.group(1) in scoped_dates and relative not in selected_paths and not (scope.voice_slugs or scope.host_slugs):
