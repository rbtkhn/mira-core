...
- State: normal commands run as `ROBERT\CodexSandboxOnline` while approved
  commands run as `ROBERT\Robert`; both contexts point at
  `C:\Users\rober\AppData\Roaming`, but only the real user context can see
