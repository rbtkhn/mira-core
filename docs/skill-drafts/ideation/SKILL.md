...
## Keep exploration and decision distinct

An open exploration is not a decision point merely because a promising idea
appears. Keep rival mechanisms and the operator's emerging purpose available;
do not substitute commercial priority or a preferred deliverable for discovery.
Recommendations required by other workflows belong to the decision handoff,
not inside the option map. When the operator asks to choose, prioritize, plan,
or implement (including within a mixed request), explicitly move to Learn
From Choices or the domain workflow and continue the authorized task. Do not
require another confirmation of an already explicit instruction. A request
only to explore can finish conversationally without a decision or save menu.

## Validate loading before scaling evaluation

For benchmark work, use `tools/run.ps1 ideation-benchmark`; read the fixtures
and [encounter checks](references/encounter-checks.md) only when evaluating.
The runner must observe the required contract reads before accepting a result;
a missing read is an invalid run, not a low creativity score. Historical v2
outputs do not establish faithful skill execution. Verify a small loading
check before an authorized larger run; do not launch the full experiment
merely to validate an installation. Passing harness tests is engineering
evidence, not demonstrated human benefit. Keep later-use assessment explicit
and operator-driven, with no automatic transcript capture or retention.
